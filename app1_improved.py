...
def format_timestamp(iso_timestamp: str) -> str:
    ...
    try:
        ...
        # If within a week
        if (now - dt).days < 7:
            return f"{(now - dt).days} days ago at {dt.strftime('%I:%M %p')}"
...
def format_member_since(iso_timestamp: str) -> str:
    """Format member registration date - shows actual date, not relative time"""
    if not iso_timestamp or iso_timestamp == "None":
...
def render_login_page():
    ...
    st.markdown(
        '<div style="text-align:right;"><a href="#" class="forgot-password">Forgot password?</a></div>',
        unsafe_allow_html=True
    )
...
def render_admin_dashboard():
    ...
    # Admin menu
    tab1, tab2, tab3, tab4 = st.tabs(["Dashboard", "Users", "Chat History", "Settings"])
    ...
    with tab3:
        ...
        st.info("💡 View aggregated chat history from all users")
        ...
        if users:
            ...
            if user:
                ...
                if not chat_history:
                    st.info(f"No chat history for {selected_user}")
                else:
                    ...
                    for chat in reversed(chat_history):
                        st.markdown(f"""
                            <div class="chat-message">
                                ...
                                <div class="timestamp">{format_timestamp(chat['timestamp'])}</div>
                            </div>
                        """, unsafe_allow_html=True)
